    ...
    def forward(self, img):
        ...
        if isinstance(img, Tensor):
            if isinstance(fill, (int, float)):
                fill = [float(fill)] * TTF.get_image_num_channels(img)
            else:
                fill = [float(f) for f in fill]

        img_size = TTF.get_image_size(img)

        ret = self.get_params(self.degrees, self.translate, self.inv_translate, self.scale, self.inv_scale, self.shear,
                              img_size)
